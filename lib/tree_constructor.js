...
function characterInHeadMode(tc, token) {
    if (isWhitespaceCharacter(token.ch))
        tc._insertCharacter(token);

    else
        inHeadModeDefaultEntry(tc, token);
}

function commentInHeadMode(tc, token) {
    tc._insertCommentNode(token);
}

function doctypeInHeadMode(tc, token) {
    tc._err('Parse error');
}

function startTagInHeadMode(tc, token) {
    if (token.tagName === 'html') {
        //TODO Process the token using the rules for the "in body" insertion mode.
    }
        ...
        tc._appendNode(tc._createElementForToken(token, true));
    }
...
function inHeadModeDefaultEntry(tc, token) {
    tc._popNodeOffOpenElementsStack();
    tc._reprocessTokenInMode(AFTER_HEAD_MODE);
}
...
TreeConstructor.prototype._popNodeOffOpenElementsStack = function () {
...
TreeConstructor.prototype._createElementForToken = function (startTagToken, selfClosingAcknowledged) {

...
function isAsciiAlphaNumeric(ch) {
    return isAsciiDigit(ch) || isAsciiUpper(ch) || isAsciiLower(ch);
}
...
Tokenizer.prototype._consumeSubsequentCharsIfMatch = function (str, startCh, caseSensitive) {
    ...
    for (var strPos = 0, ch = startCh, strCh = undefined; strPos < str.length; strPos++) {
        if (strPos > 0)
...
Tokenizer.prototype._consumeNamedEntity = function (startCh, inAttr) {
    ...
    for (; leaf && ch !== CHARS.EOF; ch = this._consumeChar(), consumedCount++, leaf = leaf.$l && leaf.$l[ch]) {
        if (leaf.$c) {
...
_[CHARACTER_REFERENCE_IN_DATA_STATE] = function (ch) {
    ...
    var entityChars = this._consumeCharacterReference(ch, false);
...
_[CHARACTER_REFERENCE_IN_ATTRIBUTE_VALUE_STATE] = function (ch) {
    var entityChars = this._consumeCharacterReference(ch, true);

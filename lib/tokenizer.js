...
//TODO use offset instead of pos
Tokenizer.prototype._lookaheadChar = function (pos) {
    var lookaheadPos = pos || (this.pos + 1),
        ch = lookaheadPos < this.html.length ? this.html[lookaheadPos] : EOF;
...
Tokenizer.prototype._isLookaheadMatchString = function (str, fromNextCh) {
    this.lookaheadMatchPos = fromNextCh ? this.pos : this.pos + 1;

    if (this.lookaheadMatchPos + str.length > this.html.length)
        return false;

    var strPos = 0,
        strCh = str[strPos],
        ch = this._lookaheadChar(this.lookaheadMatchPos);

    while (strPos < str.length) {
        if (ch !== strCh && ch !== asciiToLower(strCh))
            return false;
...
Tokenizer.prototype._consumeLastLookaheadMatch = function () {
    var seekPos = this.lookaheadMatchPos + 1;
...
Tokenizer.prototype._consumeNumberEntity = function (isHex) {
    //TODO
    throw new Error('Not implemented');
};

Tokenizer.prototype._consumeNamedEntity = function (startCh) {
    ...
    for (; ch !== EOF && leaf; ch = this._consumeChar(), consumptionExcess++, leaf = leaf.$l[ch]) {
        if (leaf.$c) {
            //NOTE: we have at least one named reference match. Reset character consumption excess counter because
            //we will not unconsume any characters before this input point for sure.
            consumptionExcess = 0;
            replacementChars = leaf.$c;

            //NOTE: we don't stop lookup at this point, because longer matches still can be found
            //(e.g. '&not' and '&notin;') except the case then found match is terminated by semicolon.
            if (ch === ';') {
        ...
        //NOTE: trie leaf doesn't contain any sub-leaves, stop lookup.
        if (!leaf.$l)
            break;
    }

    this._unconsumeSeveralChars(consumptionExcess);
...
Tokenizer.prototype._consumeCharacterReference = function (ch, inAttrValue) {
    if (ch === '\n' || ch === '\f' || ch === '\t' || ch === ' ' ||
        ch === '<' || ch === '&' || ch === EOF || ch === this.additionalAllowedCh) {
        ...
        if (isAsciiDigit(this._lookaheadChar(), isHex))
            return this._consumeNumberEntity(isHex);
        else {
            //NOTE: otherwise this is a bogus number reference. Unconsume the number sign
        ...
        return this._consumeNamedEntity(ch);
};
...
_[ATTRIBUTE_VALUE_DOUBLE_QUOTED_STATE] = function (ch) {
    if (ch === '"')
        ...
        this.returnToState = ATTRIBUTE_VALUE_DOUBLE_QUOTED_STATE;
...
_[ATTRIBUTE_VALUE_SINGLE_QUOTED_STATE] = function (ch) {
    if (ch === '\'')
        ...
        this.additionalAllowedCh = '\'';
        this.returnToState = AFTER_ATTRIBUTE_VALUE_QUOTED_STATE;
        this.state = CHARACTER_REFERENCE_IN_ATTRIBUTE_VALUE_STATE;
    }
...
_[ATTRIBUTE_VALUE_UNQUOTED_STATE] = function (ch) {
    if (ch === '\n' || ch === '\f' || ch === '\t' || ch === ' ')
        ...
        this.additionalAllowedCh = '>';
        this.returnToState = ATTRIBUTE_VALUE_UNQUOTED_STATE;
        this.state = CHARACTER_REFERENCE_IN_ATTRIBUTE_VALUE_STATE;
    }
...
_[AFTER_DOCTYPE_NAME_STATE] = function (ch) {
    ...
    else if (this._isLookaheadMatchString('PUBLIC', false)) {
        this._consumeLastLookaheadMatch();
        this.state = AFTER_DOCTYPE_PUBLIC_KEYWORD_STATE;
    }

    else if (this._isLookaheadMatchString('SYSTEM', false)) {
        this._consumeLastLookaheadMatch();

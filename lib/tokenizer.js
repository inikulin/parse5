...
//Replacement characters for numeric entities
...
var Tokenizer = exports.Tokenizer = function (html) {
    ...
    this.pendingCharacterToken = null;
...
Tokenizer.prototype._emitCurrentToken = function () {
    this._emitPendingCharacterToken();
...
Tokenizer.prototype._emitPendingCharacterToken = function () {
    if (this.pendingCharacterToken) {
        this.tokenQueue.push(this.pendingCharacterToken);
        this.pendingCharacterToken = null;
    }
};
...
Tokenizer.prototype._emitCharacter = function (ch) {
    var type = Tokenizer.CHARACTER_TOKEN;

    if (ch === unicode.NULL_CHARACTER)
        type = Tokenizer.NULL_CHARACTER_TOKEN;

    else if (isWhitespaceCharacter(ch))
        type = Tokenizer.WHITESPACE_CHARACTER_TOKEN;

    if (this.pendingCharacterToken && this.pendingCharacterToken.type !== type)
        this._emitPendingCharacterToken();

    if (this.pendingCharacterToken)
        this.pendingCharacterToken.ch += ch;

    else {
        this.pendingCharacterToken = {
            type: type,
            ch: ch
        };
    }
...
Tokenizer.prototype._emitSeveralCharacters = function (chars) {
    for (var i = 0; i < chars.length; i++)
        this._emitCharacter(chars[i]);
};
...
Tokenizer.prototype._consumeNumericEntity = function (isHex) {
    ...
    var entityCp = parseInt(digits, isHex ? 16 : 10),
        replacement = NUMERIC_ENTITY_REPLACEMENTS[entityCp];

    if (replacement)
        return replacement;

    if (isReservedCodePoint(entityCp))
...
_[CHARACTER_REFERENCE_IN_DATA_STATE] = function characterReferenceInDataState(cp) {
    ...
    if (entityChars)
        this._emitSeveralCharacters(entityChars);
    else
        this._emitCharacter('&');
};
...
_[RCDATA_STATE] = function rcdataState(cp) {
    if (cp === $.AMPERSAND)
        ...
        this._emitCharacter(unicode.REPLACEMENT_CHARACTER);
...
_[RCDATA_END_TAG_NAME_STATE] = function rcdataEndTagNameState(cp) {
    if (isAsciiUpper(cp)) {
        ...
        this._emitCharacter('<');
        this._emitCharacter('/');
        this._emitSeveralCharacters(this.tempBuff);
        this._reconsumeInState(RCDATA_STATE);
    }
};

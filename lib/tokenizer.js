...
function getStringFromCharCode(charCode) {
    //NOTE: despite the fact that JavaScript uses UTF-16 strings, String.fromCharCode() works only with a
    //UCS-2 character subset.
...
Tokenizer.prototype._lookahead = function (pos) {
...
Tokenizer.prototype._isLookaheadMatchString = function (str, fromNextCh) {
    ...
    var strPos = 0,
        strCh = str[strPos],
        ch = this._lookahead(this.lookaheadMatchPos);
...
Tokenizer.prototype._consumeCharacterReference = function (startCh, inAttr) {
    if (startCh === '\n' || startCh === '\f' || startCh === '\t' || startCh === ' ' ||
        ...
        //NOTE: we have a number entity candidate, now we should determine if it's hex or decimal
        var isHex = false,
            nextCh = this._lookahead();
        ...
        //NOTE: if we have at least one digit this is a number entity for sure, so we consume it

...
Tokenizer.prototype._consumeIfMatch = function (str, caseSensitive) {
    ...
    for (; ch !== CHARS.EOF && strPos < str.length; ch = this._consumeChar(), strCh = str[++strPos]) {
...
Tokenizer.prototype._consumeNamedEntity = function (startCh, inAttr) {
    ...
    for (; ch !== CHARS.EOF; ch = this._consumeChar(), consumedCount++, leaf = leaf.$l && leaf.$l[ch]) {
        //NOTE: trie leaf was not found for the current character. We abandon found entity chars (if we have one)

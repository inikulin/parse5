...
Tokenizer.prototype._consume = function () {
    this.consumptionPos++;
    return this.parsingFrame.advanceAndPeekCodePoint();
};
...
Tokenizer.prototype._consumeSubsequentIfMatch = function (sequence, startCp, caseSensitive) {
    ...
    for (; seqPos < seqLength; seqPos++) {
        ...
        seqCp = sequence[seqPos];

        if (cp !== seqCp && (caseSensitive || cp !== toAsciiLowerCodePoint(seqCp))) {
            isMatch = false;
            break;
        }
    }
...
Tokenizer.prototype._lookahead = function () {
    var cp = this.parsingFrame.advanceAndPeekCodePoint();
    this.parsingFrame.retreat();
...
Tokenizer.prototype._consumeNumericEntity = function (isHex) {
    ...
    } while (nextCp !== $.EOF && isDigit(nextCp));
...
_[CHARACTER_REFERENCE_IN_DATA_STATE] = function (cp) {
    this.state = DATA_STATE;
    this.additionalAllowedCp = $.UNDEFINED;
...
_[CHARACTER_REFERENCE_IN_RCDATA_STATE] = function (cp) {
...
_[SCRIPT_DATA_STATE] = function (cp) {
    if (cp === $.LESS_THAN_SIGN)
...
_[END_TAG_OPEN_STATE] = function (cp) {
    if (isAsciiUpper(cp)) {
...
_[RCDATA_LESS_THAN_SIGN_STATE] = function (cp) {
    if (cp === $.SOLIDUS) {
        this.tempBuff = '';
...
_[RCDATA_END_TAG_OPEN_STATE] = function (cp) {
    if (isAsciiUpper(cp)) {
        this._createEndTagToken(toAsciiLowerChar(cp));
...
_[RCDATA_END_TAG_NAME_STATE] = function (cp) {
    if (isAsciiUpper(cp)) {
        this.currentToken.tagName += toAsciiLowerChar(cp);
        this.tempBuff += toChar(cp);
    }
...
_[RAWTEXT_LESS_THAN_SIGN_STATE] = function (cp) {
    if (cp === $.SOLIDUS) {
        this.tempBuff = '';
...
_[RAWTEXT_END_TAG_OPEN_STATE] = function (cp) {
    if (isAsciiUpper(cp)) {
        this._createEndTagToken(toAsciiLowerChar(cp));
        this.tempBuff += toChar(cp);
...
_[RAWTEXT_END_TAG_NAME_STATE] = function (cp) {
    if (isAsciiUpper(cp)) {
        this.currentToken.tagName += toAsciiLowerChar(cp);
        this.tempBuff += toChar(cp);
    }
...
_[SCRIPT_DATA_LESS_THAN_SIGN_STATE] = function (cp) {
    if (cp === $.SOLIDUS) {
        this.tempBuff = '';
...
_[SCRIPT_DATA_END_TAG_OPEN_STATE] = function (cp) {
    if (isAsciiUpper(cp)) {
        this._createEndTagToken(toAsciiLowerChar(cp));
        this.tempBuff += toChar(cp);
...
_[SCRIPT_DATA_END_TAG_NAME_STATE] = function (cp) {
    if (isAsciiUpper(cp)) {
        this.currentToken.tagName += toAsciiLowerChar(cp);
        this.tempBuff += toChar(cp);
    }
...
_[SCRIPT_DATA_ESCAPE_START_STATE] = function (cp) {
    if (cp === $.HYPHEN_MINUS) {
...
_[SCRIPT_DATA_ESCAPE_START_DASH_STATE] = function (cp) {
    if (cp === $.HYPHEN_MINUS) {
...
_[SCRIPT_DATA_ESCAPED_STATE] = function (cp) {
    if (cp === $.HYPHEN_MINUS) {
...
_[SCRIPT_DATA_ESCAPED_DASH_STATE] = function (cp) {
    if (cp === $.HYPHEN_MINUS) {
        this.state = SCRIPT_DATA_ESCAPED_DASH_DASH_STATE;
        this._emitCharacterToken('-');
    }
...
_[SCRIPT_DATA_ESCAPED_DASH_DASH_STATE] = function (cp) {
    if (cp === $.HYPHEN_MINUS)
        this._emitCharacterToken('-');
...
_[SCRIPT_DATA_ESCAPED_LESS_THAN_SIGN_STATE] = function (cp) {
    if (cp === $.SOLIDUS) {
        this.tempBuff = '';
...
_[SCRIPT_DATA_ESCAPED_END_TAG_OPEN_STATE] = function (cp) {
    if (isAsciiUpper(cp)) {
        this._createEndTagToken(toAsciiLowerChar(cp));
        this.tempBuff += toChar(cp);
...
_[SCRIPT_DATA_ESCAPED_END_TAG_NAME_STATE] = function (cp) {
    if (isAsciiUpper(cp)) {
        this.currentToken.tagName += toAsciiLowerChar(cp);
        this.tempBuff += toChar(cp);
    }
...
_[SCRIPT_DATA_DOUBLE_ESCAPE_START_STATE] = function (cp) {
...
_[SCRIPT_DATA_DOUBLE_ESCAPED_STATE] = function (cp) {
    if (cp === $.HYPHEN_MINUS) {
...
_[SCRIPT_DATA_DOUBLE_ESCAPED_DASH_STATE] = function (cp) {
    if (cp === $.HYPHEN_MINUS) {
...
_[SCRIPT_DATA_DOUBLE_ESCAPED_DASH_DASH_STATE] = function (cp) {
    if (cp === $.HYPHEN_MINUS)
        this._emitCharacterToken('-');
...
_[SCRIPT_DATA_DOUBLE_ESCAPED_LESS_THAN_SIGN_STATE] = function (cp) {
    if (cp === $.SOLIDUS) {
        this.tempBuff = '';
...
_[SCRIPT_DATA_DOUBLE_ESCAPE_END_STATE] = function (cp) {
    if (isWhitespace(cp) || cp === $.SOLIDUS || cp === $.GREATER_THAN_SIGN) {
...
_[BEFORE_ATTRIBUTE_NAME_STATE] = function (cp) {
    if (isWhitespace(cp))
        return;
...
_[ATTRIBUTE_NAME_STATE] = function (cp) {
    if (isWhitespace(cp))
...
_[AFTER_ATTRIBUTE_NAME_STATE] = function (cp) {
    if (isWhitespace(cp))
        return;
...
_[BEFORE_ATTRIBUTE_VALUE_STATE] = function (cp) {
    if (isWhitespace(cp))
        return;
...
_[ATTRIBUTE_VALUE_DOUBLE_QUOTED_STATE] = function (cp) {
    if (cp === $.QUOTATION_MARK)
...
_[ATTRIBUTE_VALUE_SINGLE_QUOTED_STATE] = function (cp) {
    if (cp === $.APOSTROPHE)
        this.state = AFTER_ATTRIBUTE_VALUE_QUOTED_STATE;
...
_[ATTRIBUTE_VALUE_UNQUOTED_STATE] = function (cp) {
    if (isWhitespace(cp))
        this.state = BEFORE_ATTRIBUTE_NAME_STATE;
...
_[CHARACTER_REFERENCE_IN_ATTRIBUTE_VALUE_STATE] = function (cp) {
...
_[AFTER_ATTRIBUTE_VALUE_QUOTED_STATE] = function (cp) {
    if (isWhitespace(cp))
        this.state = BEFORE_ATTRIBUTE_NAME_STATE;
...
_[SELF_CLOSING_START_TAG_STATE] = function (cp) {
    if (cp === $.GREATER_THAN_SIGN) {
...
_[BOGUS_COMMENT_STATE] = function (cp) {
...
_[MARKUP_DECLARATION_OPEN_STATE] = function (cp) {
    ...
    else if (this.allowCDATA && this._consumeSubsequentIfMatch($$.CDATA_START_STRING, true))
...
_[COMMENT_START_STATE] = function (cp) {
    if (cp === $.HYPHEN_MINUS)
...
_[COMMENT_START_DASH_STATE] = function (cp) {
    if (cp === $.HYPHEN_MINUS)
...
_[COMMENT_END_DASH_STATE] = function (cp) {
    if (cp === $.HYPHEN_MINUS)
        this.state = COMMENT_END_STATE;
...
_[COMMENT_END_STATE] = function (cp) {
    if (cp === $.GREATER_THAN_SIGN) {
        this.state = DATA_STATE;
        this._emitCurrentToken();
    }
...
_[COMMENT_END_BANG_STATE] = function (cp) {
    if (cp === $.HYPHEN_MINUS) {
        this.currentToken.data += '--!';
        this.state = COMMENT_END_DASH_STATE;
    }
...
_[BEFORE_DOCTYPE_NAME_STATE] = function (cp) {
    if (isWhitespace(cp))
        return;
...
_[DOCTYPE_NAME_STATE] = function (cp) {
    if (isWhitespace(cp))
...
_[AFTER_DOCTYPE_NAME_STATE] = function (cp) {
    if (isWhitespace(cp))
        return;
...
_[AFTER_DOCTYPE_PUBLIC_KEYWORD_STATE] = function (cp) {
    if (isWhitespace(cp))
...
_[BEFORE_DOCTYPE_PUBLIC_IDENTIFIER_STATE] = function (cp) {
    if (isWhitespace(cp))
        return;
...
_[DOCTYPE_PUBLIC_IDENTIFIER_DOUBLE_QUOTED_STATE] = function (cp) {
    if (cp === $.QUOTATION_MARK)
...
_[DOCTYPE_PUBLIC_IDENTIFIER_SINGLE_QUOTED_STATE] = function (cp) {
    if (cp === $.APOSTROPHE)
        this.state = AFTER_DOCTYPE_PUBLIC_IDENTIFIER_STATE;
...
_[AFTER_DOCTYPE_PUBLIC_IDENTIFIER_STATE] = function (cp) {
    if (isWhitespace(cp))
...
_[BETWEEN_DOCTYPE_PUBLIC_AND_SYSTEM_IDENTIFIERS_STATE] = function (cp) {
    if (isWhitespace(cp))
        return;
...
_[AFTER_DOCTYPE_SYSTEM_KEYWORD_STATE] = function (cp) {
    if (isWhitespace(cp))
...
_[BEFORE_DOCTYPE_SYSTEM_IDENTIFIER_STATE] = function (cp) {
    if (isWhitespace(cp))
        return;
...
_[DOCTYPE_SYSTEM_IDENTIFIER_DOUBLE_QUOTED_STATE] = function (cp) {
    if (cp === $.QUOTATION_MARK)
...
_[DOCTYPE_SYSTEM_IDENTIFIER_SINGLE_QUOTED_STATE] = function (cp) {
    if (cp === $.APOSTROPHE)
        this.state = AFTER_DOCTYPE_SYSTEM_IDENTIFIER_STATE;
...
_[AFTER_DOCTYPE_SYSTEM_IDENTIFIER_STATE] = function (cp) {
    if (isWhitespace(cp))
        return;
...
_[BOGUS_DOCTYPE_STATE] = function (cp) {
    if (cp === $.GREATER_THAN_SIGN) {
        this._emitCurrentToken();
        this.state = DATA_STATE;
    }
...
_[CDATA_SECTION_STATE] = function (cp) {
    while (true) {
        if (cp === $.EOF) {
            this._reconsumeInState(DATA_STATE);
            break;

...
Tokenizer.prototype._lookaheadCh = function (pos) {
...
Tokenizer.prototype._isLookaheadMatchStr = function (str, fromNextCh) {
    ...
    var strPos = 0,
        strCh = str[strPos],
        ch = this._lookaheadCh(this.lookaheadMatchPos);
...
Tokenizer.prototype._consumeNamedEntity = function (startCh) {
    //TODO reformat trie and include semicolon
    var chars = null,
        ch = startCh,
        currentLeaf = namedEntitiesTrie[ch],
        consumedCount = 1;
    ...
    while (currentLeaf) {
        ...
        ch = this._consumeNextCh();
        consumedCount++;

        if (!currentLeaf.l || ch === EOF)
            break;

        currentLeaf = currentLeaf.l[ch];
    }

    //TODO unconsume all characters after last entity
    if (chars && ch === ';')
        return chars;
...
_[CHARACTER_REFERENCE_IN_DATA_STATE] = function (ch) {
    ...
    var chars = this._consumeCharacterReference(ch, false);

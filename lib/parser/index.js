...
//ParserStream
var ParserStream = module.exports = function (options, fragmentMode) {
    WritableStream.call(this);

    options = mergeOptions(DEFAULT_OPTIONS, options);

    this.treeAdapter = options.treeAdapter;
    ...
    //NOTE: For fragment parsing we create fake element which will be used as 'document'.
    //This is important for jsdom there 'document' can't be recreated, therefore
    //fragment parsing causes messing of the main `document`.
    this.document = fragmentMode ?
        this.treeAdapter.createElement('documentmock', NS.HTML, []) :
...
// Static API
ParserStream.parseFragment = function (html, fragmentContext, options) {
    var parser = new ParserStream(options, true);

    //NOTE: use <template> element as a fragment context if context element was not provided,
    //so we will parse in "forgiving" manner
    parser.fragmentContext = fragmentContext || parser.treeAdapter.createElement($.TEMPLATE, NS.HTML, []);
    ...
    var rootElement = parser.treeAdapter.getFirstChild(parser.document),
        fragment = parser.treeAdapter.createDocumentFragment();
...
//Text parsing
ParserStream.prototype._setupTokenizerCDATAMode = function () {
    var current = this._getAdjustedCurrentElement();

    this.tokenizer.allowCDATA = current && current !== this.document &&
                                this.treeAdapter.getNamespaceURI(current) !== NS.HTML && !this._isHtmlIntegrationPoint(current) && !this._isMathMLTextIntegrationPoint(current);
};

ParserStream.prototype._switchToTextParsing = function (currentToken, nextTokenizerState) {
...
//Fragment parsing
ParserStream.prototype._getAdjustedCurrentElement = function () {
    return this.openElements.stackTop === 0 && this.fragmentContext ?
        this.fragmentContext :
        this.openElements.current;
};

ParserStream.prototype._findFormInFragmentContext = function () {
...
ParserStream.prototype._initTokenizerForFragmentParsing = function () {
...
//Tree mutation
ParserStream.prototype._setDocumentType = function (token) {
    this.treeAdapter.setDocumentType(this.document, token.name, token.publicId, token.systemId);
};

ParserStream.prototype._attachElementToTree = function (element) {
...
ParserStream.prototype._appendElement = function (token, namespaceURI) {
    var element = this.treeAdapter.createElement(token.tagName, namespaceURI, token.attrs);

    this._attachElementToTree(element);
};

ParserStream.prototype._insertElement = function (token, namespaceURI) {
    var element = this.treeAdapter.createElement(token.tagName, namespaceURI, token.attrs);
...
ParserStream.prototype._insertTemplate = function (token) {
...
ParserStream.prototype._insertFakeRootElement = function () {
...
ParserStream.prototype._appendCommentNode = function (token, parent) {
    var commentNode = this.treeAdapter.createCommentNode(token.data);

    this.treeAdapter.appendChild(parent, commentNode);
};

ParserStream.prototype._insertCharacters = function (token) {
    if (this._shouldFosterParentOnInsertion())
...
ParserStream.prototype._adoptNodes = function (donor, recipient) {
    while (true) {
...
//Token processing
ParserStream.prototype._shouldProcessTokenInForeignContent = function (token) {
    var current = this._getAdjustedCurrentElement();
...
ParserStream.prototype._processToken = function (token) {
    _[this.insertionMode][token.type](this, token);
};

ParserStream.prototype._processTokenInBodyMode = function (token) {
    _[IN_BODY_MODE][token.type](this, token);
};

ParserStream.prototype._processTokenInForeignContent = function (token) {
...
ParserStream.prototype._processFakeStartTagWithAttrs = function (tagName, attrs) {
...
ParserStream.prototype._processFakeStartTag = function (tagName) {
    var fakeToken = this.tokenizer.buildStartTagToken(tagName);
...
ParserStream.prototype._processFakeEndTag = function (tagName) {
...
//Integration points
ParserStream.prototype._isMathMLTextIntegrationPoint = function (element) {
...
ParserStream.prototype._isHtmlIntegrationPoint = function (element) {
    var tn = this.treeAdapter.getTagName(element),
        ns = this.treeAdapter.getNamespaceURI(element),
...
ParserStream.prototype._reconstructActiveFormattingElements = function () {
...
//Close elements
ParserStream.prototype._closeTableCell = function () {
...
ParserStream.prototype._closePElement = function () {
    this.openElements.generateImpliedEndTagsWithExclusion($.P);
    this.openElements.popUntilTagNamePopped($.P);
};

//Insertion modes
ParserStream.prototype._resetInsertionMode = function () {
...
ParserStream.prototype._resetInsertionModeForSelect = function (selectIdx) {
    if (selectIdx > 0) {
...
ParserStream.prototype._pushTmplInsertionMode = function (mode) {
    this.tmplInsertionModeStack.push(mode);
    this.tmplInsertionModeStackTop++;
    this.currentTmplInsertionMode = mode;
};

ParserStream.prototype._popTmplInsertionMode = function () {
...
//Foster parenting
ParserStream.prototype._isElementCausesFosterParenting = function (element) {
    var tn = this.treeAdapter.getTagName(element);

    return tn === $.TABLE || tn === $.TBODY || tn === $.TFOOT || tn == $.THEAD || tn === $.TR;
};

ParserStream.prototype._shouldFosterParentOnInsertion = function () {
    return this.fosterParentingEnabled && this._isElementCausesFosterParenting(this.openElements.current);
};

ParserStream.prototype._findFosterParentingLocation = function () {
    var location = {
        parent: null,
        beforeElement: null
    };
...
ParserStream.prototype._fosterParentElement = function (element) {
...
ParserStream.prototype._fosterParentText = function (chars) {
    var location = this._findFosterParentingLocation();
...
//Special elements
ParserStream.prototype._isSpecialElement = function (element) {
    var tn = this.treeAdapter.getTagName(element),
        ns = this.treeAdapter.getNamespaceURI(element);

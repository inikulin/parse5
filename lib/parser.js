...
startTagInTableHandlers[$.INPUT] = function (p, token) {
    if (getTokenAttr(token, html.TYPE_ATTR).toLowerCase() === html.TYPE_ATTR_HIDDEN_VALUE) {
        p._err('Parse error');
        p._appendElementForToken(token);
    }
...
startTagInTableHandlers[$.FORM] = function (p, token) {
    p._err('Parse error');
...
Parser.prototype._processFakeStartTag = function (tagName) {
    var fakeToken = this.tokenizer.createFakeStartTagToken(tagName);
...
Parser.prototype._processFakeEndTag = function (tagName) {
    var fakeToken = this.tokenizer.createFakeEndTagToken(tagName);

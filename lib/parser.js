var Tokenizer = require('./tokenizer').Tokenizer,
    ElementStack = require('./element_stack').ElementStack,
...
function startTagBeforeHead(p, token) {
    ...
    if (tn === $.HTML)
        ...
        p._insertElementForToken(token, false);
        p.headElement = p.openElements.currentNode;
...
startTagInBodyProcessors[$.HTML] = function (p, token) {
    p._err('Parse error');
    p.treeBuilder.adoptAttributes(p.openElements.currentNode, token.attrs);
};
...
startTagInBodyProcessors[$.BODY] = function (p, token) {
    ...
    var properlyInsertedBody = p.openElements.peekProperlyInsertedBodyElement();
...
startTagInBodyProcessors[$.FRAMESET] = function (p, token) {
    ...
    if (p.openElements.peekProperlyInsertedBodyElement()) {
        p.treeBuilder.detachNode(p.openElements.currentNode);
        p.openElements.popUpToHtmlElement();
        p._insertElementForToken(token, false);
        p.insertionMode = IN_FRAMESET_MODE;
    }
};
...
startTagInBodyProcessors[$.H6] = function (p, token) {
    ...
    var currNodeTn = p.openElements.currentNode.tagName;

    if (currNodeTn === $.H1 || currNodeTn === $.H2 || currNodeTn === $.H3 ||
        currNodeTn === $.H4 || currNodeTn === $.H5 || currNodeTn === $.H6) {
        p._err('Parse error');
        p.openElements.pop();
    }
...
startTagInBodyProcessors[$.FORM] = function (p, token) {
    if (p.formElement)
        ...
        p._insertElementForToken(token, false);
        p.formElement = p.openElements.currentNode;
    }
};
...
startTagInBodyProcessors[$.DT] = function (p, token) {
    ...
    for (var i = p.openElements.stackTop; i >= 0; i--) {
        ...
        if (tn !== $.ADDRESS && tn !== $.DIV && tn !== $.P && idioms.isSpecial(element))
            break;
...
function startTagInBody(p, token) {
    var processor = startTagInBodyProcessors[token.tagName];
...
var Parser = exports.Parser = function (html, treeBuilder) {
    ...
    this.openElements = new ElementStack(this.document);
...
Parser.prototype._insertElement = function (element) {
    this.treeBuilder.appendNode(this.openElements.currentNode, element);
...
Parser.prototype._appendCommentNode = function (token) {
    ...
    this.treeBuilder.appendNode(this.openElements.currentNode, commentNode);
};

Parser.prototype._insertCharacter = function (token) {
    this.treeBuilder.insertCharacterToNode(this.openElements.currentNode, token.ch);
};

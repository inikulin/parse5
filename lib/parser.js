...
Parser.prototype._insertElement = function (token, namespaceURI) {
    ...
    if (this.fosterParentingEnabled && this._isElementCausesFosterParenting(this.openElements._current))
...
Parser.prototype._shouldProcessTokenInCurrentInsertionMode = function (token) {
    return (this.openElements.current === this.document ||
...
Parser.prototype._adoptionAgencyInnerLoop = function (furthestBlock, formattingElement) {
    ...
    for (var i = 0; i < 3; i++) {
        ...
        if (lastElement === furthestBlock)
            this.activeFormattingElements.moveBookmarkAfterEntry(elementEntry);

        this.treeAdapter.detachNode(lastElement);
        this.treeAdapter.appendNode(element, lastElement);
        lastElement = element;
    }
...
Parser.prototype._callAdoptionAgency = function (token) {
    for (var i = 0; i < 8; i++) {
        ...
        this.activeFormattingElements.createBookmark(formattingElement);

        var lastElement = this._adoptionAgencyInnerLoop(furthestBlock, formattingElement),
            commonAncestor = this.openElements.getCommonAncestor(formattingElement);
        ...
        if (this._isElementCausesFosterParenting(commonAncestor))
            ...
            this.treeAdapter.appendNode(commonAncestor, lastElement);
        //TODO
    }
};
...
function inTableModeDefaultHandler(p, token) {
    var savedFosterParentingState = this.fosterParentingEnabled;

    p._processTokenInBodyMode(token);
    this.fosterParentingEnabled = savedFosterParentingState;
}

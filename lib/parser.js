...
startTagInBodyHandlers[$.OPTION] = function (p, token) {
    if (p.openElements.current.tagName === $.OPTION) {
        //TODO act as if an end tag with the tag name "option" had been seen.
    }
...
endTagInBodyHandlers[$.BODY] = function (p, token) {
    ...
    if (!p.openElements.hasInScope($.BODY)) {
        ...
        for (var i = p.openElements.stackTop; i >= 0; i--) {
            ...
            if (tn !== $.DD || tn !== $.DT || tn !== $.LI || tn !== $.OPTGROUP || tn !== $.OPTION || tn !== $.P ||
                tn !== $.RP || tn !== $.RT || tn !== $.TBODY || tn !== $.TD || tn !== $.TFOOT || tn !== $.TH ||
                tn !== $.THEAD || tn !== $.TR || tn !== $.BODY || tn !== $.HTML) {
                p._err('Parse error');
                break;
            }
        }
...
endTagInBodyHandlers[$.HTML] = function (p, token) {
    if (endTagInBodyHandlers[$.BODY](p, token))
        p._reprocessTokenInMode(token, AFTER_BODY_MODE);
};

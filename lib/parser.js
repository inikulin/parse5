...
Parser.prototype._shouldProcessTokenInForeignContent = function (token) {
    ...
    if (currentElement === this.document || currentElement.namespaceURI === html.NAMESPACES.HTML)
        return false;

    if (idioms.isElementInMathMLTextIntegrationPoint(currentElement) &&
        (token.type === Tokenizer.CHARACTER_TOKEN ||
         (token.type === Tokenizer.START_TAG_TOKEN && token.tagName !== $.MGLYPH && token.tagName !== $.MALIGNMARK))) {
        return false;
    }

    if (currentElement.tagName === $.ANNOTATION_XML &&
        currentElement.namespaceURI === html.NAMESPACES.MATHML &&
        token.type === Tokenizer.START_TAG_TOKEN && token.tagName === $.SVG) {
        return false;
    }

    if (idioms.isElementInHtmlIntegrationPoint(currentElement) &&

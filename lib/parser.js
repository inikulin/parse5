...
Parser.prototype._appendHtmlElementForToken = function (token) {
    var element = this._createElementForToken(token, html.NAMESPACES.HTML, true);
    this.treeAdapter.appendNode(this.openElements.current, element);
};

Parser.prototype._insertHtmlElementForToken = function (token) {
    var element = this._createElementForToken(token, html.NAMESPACES.HTML, false);
    this.treeAdapter.appendNode(this.openElements.current, element);
    this.openElements.push(element);
};

Parser.prototype._insertFakeRootElement = function () {
    var element = this.treeAdapter.createElement($.HTML, [], html.NAMESPACES.HTML);
    this.treeAdapter.appendNode(this.openElements.current, element);
    this.openElements.push(element);
};
...
Parser.prototype._shouldProcessTokenInForeignContent = function (token) {
    ...
    if (currentElement === this.document || this.openElements.currentNamespaceURI === html.NAMESPACES.HTML)
        return false;

    if (this.openElements.isInMathMLTextIntegrationPoint() &&
    ...
    if (this.openElements.currentTagName === $.ANNOTATION_XML &&
        this.openElements.currentNamespaceURI === html.NAMESPACES.MATHML &&
        token.type === Tokenizer.START_TAG_TOKEN && token.tagName === $.SVG) {
        return false;
    }

    if (this.openElements.isInHtmlIntegrationPoint() &&
...
Parser.prototype._generateImpliedEndTags = function () {
    while (idioms.REQUIRES_IMPLIED_END_TAG[this.openElements.currentTagName])
        this.openElements.pop();
};

Parser.prototype._generateImpliedEndTagsWithExclusion = function (exclusionTagName) {
    while (idioms.REQUIRES_IMPLIED_END_TAG[this.openElements.currentTagName] &&
           this.openElements.currentTagName !== exclusionTagName) {
        this.openElements.pop();
    }
};

...
Parser.prototype._shouldProcessTokenInForeignContent = function (token) {
    var currentElement = this.openElements.current,
        ceTagName = currentElement.tagName,
        ceNamespaceURI = currentElement.namespaceURI;

    if (currentElement === this.document || ceNamespaceURI === html.NAMESPACES.HTML)
        return false;

    if (idioms.isInMathMLTextIntegrationPoint(ceTagName, ceNamespaceURI) &&
        (token.type === Tokenizer.CHARACTER_TOKEN ||
         (token.type === Tokenizer.START_TAG_TOKEN && token.tagName !== $.MGLYPH && token.tagName !== $.MALIGNMARK))) {
        return false;
    }

    if (ceTagName === $.ANNOTATION_XML && ceNamespaceURI === html.NAMESPACES.MATHML &&
        token.type === Tokenizer.START_TAG_TOKEN && token.tagName === $.SVG) {
        return false;
    }

    if (idioms.isInHtmlIntegrationPoint(ceTagName, ceNamespaceURI) &&
        (token.type === Tokenizer.START_TAG_TOKEN || token.type === Tokenizer.END_TAG_TOKEN)) {
        return false;
    }
...
Parser.prototype._appendCommentNodeToRootHtmlElement = function (token) {
    var commentNode = this.treeBuilder.createCommentNode(token.data);
    this.treeBuilder.appendNode(this.openElements.stack[0], commentNode);
};

Parser.prototype._appendCommentNodeToDocument = function (token) {
    var commentNode = this.treeBuilder.createCommentNode(token.data);
    this.treeBuilder.appendNode(this.document, commentNode);
};
...
//Adoption agency alogrithm (see: http://www.whatwg.org/specs/web-apps/current-work/multipage/tree-construction.html#adoptionAgency)
...
endTagInBodyHandlers[$.P] = function pEndTagInBody(p, token) {
    if (p.openElements.hasInButtonScope($.P)) {
        p._generateImpliedEndTagsWithExclusion($.P);
...
function startTagInSelect(p, token) {
    ...
    if (tn === $.HTML)
        ...
        if (p.openElements.current.tagName === $.OPTGROUP)
...
function endTagInSelect(p, token) {
    ...
    if (tn === $.OPTGROUP) {
        ...
        if (p.openElements.current.tagName === $.OPTION && prevOpenElement && prevOpenElement.tagName === $.OPTGROUP)
            p._processFakeEndTag($.OPTION);

...
//TODO this is ugly and ineffective. Do stmhg =)
function adoptAttributes(recipientNode, attrs) {
...
function startTagInBody(p, token) {
    ...
    if (tn === 'html') {
        p._err('Parse error');
        adoptAttributes(p.currentNode, token.attrs);
    }
        ...
        if (secondOpenElement && secondOpenElement.tagName === 'body') {
            p.framesetOk = false;
            adoptAttributes(secondOpenElement, token.attrs);
        }
    }

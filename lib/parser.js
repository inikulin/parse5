...
startTagInBodyProcessors[$.UL] = function (p, token) {
    if (p.openElementsStack.hasElementInButtonScope($.P)) {
...
startTagInBodyProcessors[$.H6] = function (p, token) {
    ...
    var currNodeTn = p.openElementsStack.currentNode.tagName;
...
startTagInBodyProcessors[$.FORM] = function (p, token) {
    if (p.formElement)
        ...
        p._insertElementForToken(token, false);
        p.formElement = p.openElementsStack.currentNode;
    }
};
...
startTagInBodyProcessors[$.DT] = function (p, token) {
    ...
    for (var i = p.openElementsStack.stackTop; i >= 0; i--) {
        var element = p.openElementsStack.stack[i],
            tn = element.tagName;

        if (tn !== $.ADDRESS && tn !== $.DIV && tn !== $.P && elements.isSpecial(element))
            break;
...
startTagInBodyProcessors[$.PLAINTEXT] = function (p, token) {
    ...
    p._insertElementForToken(token, false);
    p.tokenizer.switchToPLAINTEXTState();
};

startTagInBodyProcessors[$.BUTTON] = function (p, token) {
    if (p.openElementsStack.hasElementInScope($.BUTTON)) {
        p._err('Parse error');
...
Parser.prototype._createElementForToken = function (token, selfClosingAcknowledged) {
    ...
    return this.treeBuilder.createElement(token.tagName, token.attrs, NAMESPACES.HTML);
};
...
Parser.prototype._insertCharacter = function (token) {
    this.treeBuilder.insertCharacterToNode(this.openElementsStack.currentNode, token.ch);
};

...
Parser.prototype.parse = function () {
    while (!this.stopped) {
        ...
        if (this._shouldProcessTokenInCurrentInsertionMode(token))
...
//Tree mutation
Parser.prototype._appendDoctype = function (token) {
    var doctype = this.treeAdapter.createDocumentType(token.name, token.publicId, token.systemId);

    this.treeAdapter.appendChild(this.document, doctype);
};
...
Parser.prototype._insertCharacter = function (token) {
    if (this.fosterParentingEnabled && this._isElementCausesFosterParenting(this.openElements.current)) {
        ...
        if (location.beforeElement)
            this.treeAdapter.insertCharacterBefore(location.parent, token.ch, location.beforeElement);
        else
            this.treeAdapter.insertCharacter(location.parent, token.ch);
    }
    else
        this.treeAdapter.insertCharacter(this.openElements.current, token.ch);
};

//Token processing
Parser.prototype._shouldProcessTokenInCurrentInsertionMode = function (token) {
    return (this.openElements.currentNamespaceURI === NS.HTML || !this.openElements.current ||
            ...
            (this.openElements.currentTagName === $.ANNOTATION_XML &&
             this.openElements.currentNamespaceURI === NS.MATHML &&
             token.type === Tokenizer.START_TAG_TOKEN && token.tagName === $.SVG) ||

            (this.openElements.isHtmlIntegrationPoint() && (token.type === Tokenizer.START_TAG_TOKEN ||
...
Parser.prototype._recreateElementForAdoptionAgency = function (elementEntry) {
    ...
    var ns = this.treeAdapter.getElementNamespaceURI(elementEntry.element),
...
Parser.prototype._replaceFormattingElementForAdoptionAgency = function (furthestBlock, formattingElementEntry) {
    ...
    var ns = this.treeAdapter.getElementNamespaceURI(formattingElementEntry.element),
...
Parser.prototype._findFosterParentingLocation = function () {
    ...
    for (var i = this.openElements.stackTop; i >= 0; i--) {
        ...
        if (this.treeAdapter.getElementTagName(openElement) === $.TABLE) {
    ...
    //TODO If there is no table element in the stack of open elements (fragment case), then the foster
    //parent element is the first element in the stack of open elements (the html element).
...
function endTagInForeignContent(p, token) {
    if (p.treeAdapter.getElementTagName(p.openElements.current).toLowerCase() !== token.tagName)
        p._err('Parse error');

    for (var i = p.openElements.stackTop; i > 0; i--) {
        var element = p.openElements.items[i];

        if (p.treeAdapter.getElementNamespaceURI(element) === NS.HTML) {
            p._processToken(token);
            break;
        }

        if (p.treeAdapter.getElementTagName(element).toLowerCase() === token.tagName) {
            p.openElements.popUntilElementPopped(element);
            break;
        }
    }

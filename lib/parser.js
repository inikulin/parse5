...
Parser.prototype._reset = function (html, fragmentContext) {
    ...
    this._setInsertionMode(INITIAL_MODE);
};
...
Parser.prototype._switchToTextParsing = function (currentToken, nextTokenizerState) {
    ...
    this._setInsertionMode(TEXT_MODE);
};
...
Parser.prototype._processToken = function (token) {
    this.tokenHandlers[token.type](this, token);
};
...
//Insertion mode
Parser.prototype._setInsertionMode = function (mode) {
    this.insertionMode = mode;
    this.tokenHandlers = IM_[mode];
};
...
Parser.prototype._resetInsertionModeAppropriately = function () {
    for (var i = this.openElements.stackTop, last = false; i >= 0; i--) {
        ...
        if (resetInsertionMode) {
            this._setInsertionMode(resetInsertionMode);
            break;
        }

        else if (!last && (tn === $.TD || tn === $.TH)) {
            this._setInsertionMode(IN_CELL_MODE);
            break;
        }

        else if (last) {
            this._setInsertionMode(IN_BODY_MODE);
            break;
        }
    }
};
...
function doctypeInInitialMode(p, token) {
    ...
    p._setInsertionMode(BEFORE_HTML_MODE);
}
...
function startTagBeforeHtml(p, token) {
    if (token.tagName === $.HTML) {
        ...
        p._setInsertionMode(BEFORE_HEAD_MODE);
    }
...
function startTagBeforeHead(p, token) {
    ...
    if (tn === $.HTML)
        ...
        p._setInsertionMode(IN_HEAD_MODE);
    }
...
function endTagInHead(p, token) {
    ...
    if (tn === $.HEAD) {
        p.openElements.pop();
        p._setInsertionMode(AFTER_HEAD_MODE);
    }
...
function startTagAfterHead(p, token) {
    ...
    if (tn === $.HTML)
        ...
        p._insertElement(token, NS.HTML);
        p.framesetOk = false;
        p._setInsertionMode(IN_BODY_MODE);
    }

    else if (tn === $.FRAMESET) {
        p._insertElement(token, NS.HTML);
        p._setInsertionMode(IN_FRAMESET_MODE);
    }
...
function tableStartTagInBody(p, token) {
    ...
    p._insertElement(token, NS.HTML);
    p.framesetOk = false;
    p._setInsertionMode(IN_TABLE_MODE);
}
...
function selectStartTagInBody(p, token) {
    ...
    if (p.insertionMode === IN_TABLE_MODE || p.insertionMode === IN_CAPTION_MODE ||
        ...
        p._setInsertionMode(IN_SELECT_IN_TABLE_MODE);
    }

    else
        p._setInsertionMode(IN_SELECT_MODE);
}
...
//OPTIMIZATION: It looks ugly, but it's fast. Faster than using dictionary.
//Integer comparisons are low-cost, so we can use very fast tag name length filter.
...
function bodyEndTagInBody(p, token) {
    if (p.openElements.hasInScope($.BODY))
        p._setInsertionMode(AFTER_BODY_MODE);
...
function endTagInText(p, token) {
    ...
    p.openElements.pop();
    p._setInsertionMode(p.originalInsertionMode);
}
...
function characterInTable(p, token) {
    ...
    if (curTn === $.TABLE || curTn === $.TBODY || curTn === $.TFOOT || curTn === $.THEAD || curTn === $.TR) {
        ...
        p.originalInsertionMode = p.insertionMode;
        p._setInsertionMode(IN_TABLE_TEXT_MODE);
        p._processToken(token);
    }
...
function captionStartTagInTable(p, token) {
    ...
    p.activeFormattingElements.insertMarker();
    p._insertElement(token, NS.HTML);
    p._setInsertionMode(IN_CAPTION_MODE);
}

function colgroupStartTagInTable(p, token) {
    p.openElements.clearBackToTableContext();
    p._insertElement(token, NS.HTML);
    p._setInsertionMode(IN_COLUMN_GROUP_MODE);
}
...
function tbodyStartTagGroupInTable(p, token) {
    p.openElements.clearBackToTableContext();
    p._insertElement(token, NS.HTML);
    p._setInsertionMode(IN_TABLE_BODY_MODE);
}
...
function startTagInTableBody(p, token) {
    ...
    if (tn === $.TR) {
        ...
        p._insertElement(token, NS.HTML);
        p._setInsertionMode(IN_ROW_MODE);
    }
...
function startTagInRow(p, token) {
    ...
    if (tn === $.TH || tn === $.TD) {
        ...
        p._insertElement(token, NS.HTML);
        p._setInsertionMode(IN_CELL_MODE);
        p.activeFormattingElements.insertMarker();
    }
...
function endTagAfterBody(p, token) {
    if (token.tagName === $.HTML) {
        if (!p.fragmentContext)
            p._setInsertionMode(AFTER_AFTER_BODY_MODE);
    }
...
function endTagInFrameset(p, token) {
    if (token.tagName === $.FRAMESET && !p.openElements.isRootHtmlElementCurrent()) {
        ...
        if (!p.fragmentContext && p.openElements.currentTagName !== $.FRAMESET)
            p._setInsertionMode(AFTER_FRAMESET_MODE);
    }
}
...
function endTagAfterFrameset(p, token) {
    if (token.tagName === $.HTML)
        p._setInsertionMode(AFTER_AFTER_FRAMESET_MODE);
}

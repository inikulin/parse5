...
function startTagInHead(p, token) {
    ...
    if (tn === $.HTML)
        ...
        p._parseGenericText(token, Tokenizer.RCDATA_STATE);

    //NOTE: here we assume that we always act as an interactive user agent with enabled scripting, so we parse
    //<noscript> as a rawtext.
    else if (tn === $.NOSCRIPT || tn === $.NOFRAMES || tn === $.STYLE)
        p._parseGenericText(token, Tokenizer.RAWTEXT_STATE);
...
startTagInBodyProcessors[$.A] = function (p, token) {
    //TODO
};

startTagInBodyProcessors[$.B] =
startTagInBodyProcessors[$.BIG] =
startTagInBodyProcessors[$.CODE] =
startTagInBodyProcessors[$.EM] =
startTagInBodyProcessors[$.FONT] =
startTagInBodyProcessors[$.I] =
startTagInBodyProcessors[$.S] =
startTagInBodyProcessors[$.SMALL] =
startTagInBodyProcessors[$.STRIKE] =
startTagInBodyProcessors[$.STRONG] =
startTagInBodyProcessors[$.TT] =
startTagInBodyProcessors[$.U] = function (p, token) {
    p._reconstructActiveFormattingElements();
    p._insertElementForToken(token);
    p.activeFormattingElements.push(p.openElements.current, token);
};

startTagInBodyProcessors[$.NOBR] = function (p, token) {
    p._reconstructActiveFormattingElements();

    if (p.openElements.hasInScope($.NOBR)) {
        p._err('Parse error');
        //TODO act as if an end tag with the tag name "nobr" had been seen
        p._reconstructActiveFormattingElements();
    }
...
startTagInBodyProcessors[$.APPLET] =
startTagInBodyProcessors[$.MARQUEE] =
startTagInBodyProcessors[$.OBJECT] = function (p, token) {
    p._reconstructActiveFormattingElements();
    p._insertElementForToken(token);
...
startTagInBodyProcessors[$.TABLE] = function (p, token) {
    if (!p.document.quirksMode && p.openElements.hasInButtonScope($.P)) {
        //TODO then act as if an end tag with the tag name "p" had been seen.
    }
...
startTagInBodyProcessors[$.AREA] =
startTagInBodyProcessors[$.BR] =
startTagInBodyProcessors[$.EMBED] =
startTagInBodyProcessors[$.IMG] =
startTagInBodyProcessors[$.KEYGEN] =
startTagInBodyProcessors[$.WBR] = function (p, token) {
    p._reconstructActiveFormattingElements();
    p._appendElementForToken(token);
    p.framesetOk = false;
};

startTagInBodyProcessors[$.INPUT] = function (p, token) {
    p._reconstructActiveFormattingElements();
    p._appendElementForToken(token);

    if (getTokenAttr(token, html.TYPE_ATTR_NAME).toLowerCase() === html.TYPE_ATTR_HIDDEN_VALUE)
        p.framesetOk = false;
...
Parser.prototype._parseGenericText = function (currentToken, nextTokenizerState) {

...
Parser.prototype._callAdoptionAgency = function (token) {
    for (var i = 0; i < 8; i++) {
        ...
        if (this._isElementCausesFosterParenting(commonAncestor))
            this._fosterParent(lastElement);
...
Parser.prototype._fosterParent = function (element) {
    ...
    for (var i = this.openElements.stackTop; i >= 0; i--) {
        ...
        if (this.treeAdapter.getElementTagName(openElement) === $.TABLE) {
            fosterParent = this.treeAdapter.getParentNode(openElement);

            if (fosterParent)
                insertBeforeElement = openElement;
            else
                fosterParent = this.openElements.items[i - 1];
    ...
    if (insertBeforeElement)
        this.treeAdapter.insertBefore(fosterParent, element, insertBeforeElement);
    else
        this.treeAdapter.appendChild(fosterParent, element);
};

...
//NOTE: here we assume that we already know that first code point is more than or equal to 0xD800.
function isSurrogatePair(cp1, cp2) {
    return cp1 <= 0xDBFF && cp2 >= 0xDC00 && cp2 <= 0xDFFF;
}

function isIsolatedSurrogate(cp) {
    return cp <= 0xDBFF || cp >= 0xDC00 && cp <= 0xDFFF;
}
...
Preprocessor.prototype._characterError = function (code, cp) {
    //NOTE: avoid reporting error twice for the same character
    if (this.lastErrPos !== this.pos) {
        this.lastErrPos = this.pos;

        this._err(code, {codepoint: cp});
    }
};
...
Preprocessor.prototype._processHighRangeCodePoint = function (cp) {
    ...
    if (isIsolatedSurrogate(cp) || cp > 0x10FFFF)
        this._characterError(ERR.nonUnicodeCharacterInInputStream, cp);
...
Preprocessor.prototype._validateCodePoint = function (cp) {
    ...
    if (isInvalidCp)
        this._characterError(ERR.controlOrUndefinedCharacterInInputStream, cp);
};

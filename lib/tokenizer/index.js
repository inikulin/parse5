...
_[NAMED_CHARACTER_REFERENCE_STATE] = function (cp) {
    ...
    if (this._ensureHibernation())
        ...
        if (matchResult) {
            ...
            var isSemicolon = lastConsumed === $.SEMICOLON;
            var quirkAttr = this._isCharacterReferenceAttributeQuirk();

            if (!isSemicolon && !quirkAttr)
                this._err(ERR.unexpectedCharacterAfterReferenceName);

            if (isSemicolon || !quirkAttr)
                this.tempBuff = matchResult;
        }
        else
            this._err(ERR.invalidCharacterReferenceName);
...
_[NUMERIC_CHARACTER_REFERENCE_END_STATE] = function numericCharacterReferenceEndState() {
    ...
    else if (unicode.isControlOrUndefinedCodePoint(this.charRefCode) || this.charRefCode === $.CARRIAGE_RETURN) {
        this._err(ERR.controlOrUndefinedCharacterReference);

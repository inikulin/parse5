...
Tokenizer.prototype._consumeNamedEntity = function (startCp, inAttr) {
    var referencedCodePoints = [],
        cp = startCp,
        leaf = NAMED_ENTITY_TRIE[cp],
        semicolonTerminated = false,
        nextCp = void 0;

    for (; leaf && cp !== $.EOF; cp = this._consume(), leaf = leaf.l && leaf.l[cp]) {
        this.tempBuff.push(cp);
        if (leaf.c) {
            ...
            if (inAttr && cp !== $.SEMICOLON & (nextCp === $.EQUALS_SIGN || isAsciiAlphaNumeric(nextCp)))
                continue;
    ...
    return referencedCodePoints.concat(this.tempBuff);
};

Tokenizer.prototype._consumeCharacterReference = function (ampCp, inAttr) {
    this.tempBuff = [ampCp];

    var startCp = this._lookahead();
    ...
    if (startCp === $.NUMBER_SIGN) {
        ...
        if (nextCp === $.LATIN_SMALL_X || nextCp === $.LATIN_CAPITAL_X) {
            this.tempBuff.push(this._consume());
            isHex = true;
        }
...
_[DATA_STATE] = function dataState(cp) {
    if (cp === $.AMPERSAND)
        this._reconsumeInState(CHARACTER_REFERENCE_IN_DATA_STATE);
...
_[RCDATA_STATE] = function rcdataState(cp) {
    if (cp === $.AMPERSAND)
        this._reconsumeInState(CHARACTER_REFERENCE_IN_RCDATA_STATE);
...
_[ATTRIBUTE_VALUE_DOUBLE_QUOTED_STATE] = function attributeValueDoubleQuotedState(cp) {
    if (cp === $.QUOTATION_MARK)
        ...
        this._reconsumeInState(CHARACTER_REFERENCE_IN_ATTRIBUTE_VALUE_STATE);
    }

...
var DATA_STATE = 'DATA_STATE',
    ...
    CHARACTER_REFERENCE_END_STATE = 'CHARACTER_REFERENCE_END_STATE';
...
Tokenizer.prototype._leaveAttrName = function (toState) {
    ...
    var ct = this.currentToken,
        attrName = this.currentAttr.name;

    if (!ct.attrNames[attrName]) {
        ct.attrs.push(this.currentAttr);
        ct.attrNames[attrName] = true;
    }
};
...
Tokenizer.prototype._emitCurrentToken = function () {
    ...
    if (ct.type === Tokenizer.START_TAG_TOKEN)
        ...
        if (ct.selfClosing)
            this._err(ERR.selfClosingEndTag);
    }
...
Tokenizer.prototype._isCharacterReferenceAttributeQuirk = function () {
...
_[TAG_OPEN_STATE] = function tagOpenState(cp) {
    if (cp === $.EXCLAMATION_MARK)
        ...
        this._err(ERR.startOfProcessingInstructionOrXMLDeclaration);
        this._createCommentToken();
        this._reconsumeInState(BOGUS_COMMENT_STATE);
    }

    else {
        this._err(ERR.unexpectedFirstCharacterOfStartTagName);
        this._emitChars('<');
...
_[END_TAG_OPEN_STATE] = function endTagOpenState(cp) {
    if (isAsciiLetter(cp)) {
        ...
        this._err(ERR.eofBeforeEndTagName);
        this._emitChars('</');
        this._emitEOFToken();
    }
...
_[BEFORE_ATTRIBUTE_NAME_STATE] = function beforeAttributeNameState(cp) {
    ...
    if (cp === $.SOLIDUS || cp === $.GREATER_THAN_SIGN || cp === $.EOF)
        ...
        this._err(ERR.unexpectedEqualSignBeforeAttributeName);
        this._createAttr('=');
...
_[ATTRIBUTE_NAME_STATE] = function attributeNameState(cp) {
    if (isWhitespace(cp) || cp === $.SOLIDUS || cp === $.GREATER_THAN_SIGN || cp === $.EOF) {
        ...
        this._err(ERR.invalidCharacterInAttributeName);
...
_[AFTER_ATTRIBUTE_NAME_STATE] = function afterAttributeNameState(cp) {
    ...
    if (cp === $.SOLIDUS)
        ...
        this._err(ERR.eofAfterAttributeName);
        this._emitEOFToken();
    }
...
_[MARKUP_DECLARATION_OPEN_STATE] = function markupDeclarationOpenState(cp) {
    ...
    //results are no longer valid and we will need to start over.
        ...
        this._err(ERR.malformedComment);
        this._createCommentToken();
        this._reconsumeInState(BOGUS_COMMENT_STATE);
    }
};
...
_[CHARACTER_REFERENCE_STATE] = function characterReferenceState(cp) {
    ...
    if (isWhitespace(cp) || cp === $.LESS_THAN_SIGN || cp === $.AMPERSAND || cp === $.EOF)
        this._reconsumeInState(CHARACTER_REFERENCE_END_STATE);

    else if (cp === $.NUMBER_SIGN) {
        this.tempBuff.push(cp);
...
_[NAMED_CHARACTER_REFERENCE_STATE] = function (cp) {
    ...
    if (this._ensureHibernation())
        ...
        if (matchResult) {
            var lastConsumed = this.tempBuff[this.tempBuff.length - 1];

            if (lastConsumed === $.SEMICOLON || !this._isCharacterReferenceAttributeQuirk())
                this.tempBuff = matchResult;
        }

        this.state = CHARACTER_REFERENCE_END_STATE;
    }
};
...
_[NUMERIC_CHARACTER_REFERENCE_END_STATE] = function numericCharacterReferenceEndState() {
    var replacement = NUMERIC_ENTITY_REPLACEMENTS[this.charRefCode];

    if (replacement)
        this.tempBuff = [replacement];

    else if (isReservedCodePoint(this.charRefCode))
        this.tempBuff = [$.REPLACEMENT_CHARACTER];
...
//------------------------------------------------------------------
_[CHARACTER_REFERENCE_END_STATE] = function characterReferenceEndState() {
    if (this._isCharacterReferenceInAttribute()) {
        for (var i = 0; i < this.tempBuff.length; i++)

...
Parser.prototype._insertLastNodeInCommonAncestorForAdoptionAgency = function (commonAncestor, lastElement) {
    if (this._isElementCausesFosterParenting(commonAncestor))
        this._fosterParentElement(lastElement);

    else {
        var tn = this.treeAdapter.getTagName(commonAncestor),
            ns = this.treeAdapter.getNamespaceURI(commonAncestor);

        if (tn === $.TEMPLATE && ns === NS.HTML)
            commonAncestor = this.treeAdapter.getChildNodes(commonAncestor)[0];
...
Parser.prototype._callAdoptionAgency = function (token) {
    for (var i = 0; i < 8; i++) {
        ...
        this.treeAdapter.detachNode(lastElement);
        this._insertLastNodeInCommonAncestorForAdoptionAgency(commonAncestor, lastElement);

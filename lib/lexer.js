...
//Lookahead for a string string
Lexer.prototype._lookahead = function (fromNextCh, what) {
    var htmlPos = fromNextCh ? this.pos : this.pos + 1;

    if (htmlPos + what.length > this.html.length)
        return false;

    for (var i = 0; i < what.length; i++, htmlPos++, i++) {
        if (this.html[htmlPos] !== what[i] && this.html[htmlPos] !== asciiToLower(what[i]))
            return false;
    }

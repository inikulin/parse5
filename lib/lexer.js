...
Lexer.DOCTYPE_TOKEN = 'DOCTYPE_TOKEN';
...
Lexer.prototype.getToken = function () {
    ...
    while (!this.tokenQueue.length) {
        ch = this.pos < this.html.length ? this.html[this.pos] : EOF;
...
Lexer.prototype._lookahead = function (what, fromNextCh) {
    this.lookaheadPos = fromNextCh ? this.pos : this.pos + 1;

    if (this.lookaheadPos + what.length > this.html.length)
        return false;

    for (var i = 0; i < what.length; this.lookaheadPos++, i++) {
        if (this.html[this.lookaheadPos] !== what[i] && this.html[this.lookaheadPos] !== asciiToLower(what[i]))
            return false;
    }
...
Lexer.prototype._consumeLastLookaheadStr = function () {
    this.pos = this.lookaheadPos + 1;
};
...
Lexer.prototype._createDoctypeToken = function (doctypeNameFirstCh) {
    this.currentToken = {
        type: Lexer.DOCTYPE_TOKEN,
...
//Token emission
Lexer.prototype._emitCurrentToken = function () {
    //NOTE: store emited start tag's tagName to determine is the following end tag token is appropriate.
    if (this.currentToken.type === Lexer.START_TAG_TOKEN)
...
_[DOCTYPE_STATE] = function (ch) {
    if (ch === '\n' || ch === '\r' || ch === '\f' || ch === '\t' || ch === ' ')
        ...
        this._reconsume(BEFORE_DOCTYPE_NAME_STATE);
    }
};
...
_[AFTER_DOCTYPE_NAME_STATE] = function (ch) {
    ...
    else if (this._lookahead('PUBLIC', false)) {
        this._consumeLastLookaheadStr();
        this.state = AFTER_DOCTYPE_PUBLIC_KEYWORD_STATE;
    }

    else if (this._lookahead('SYSTEM', false)) {
        this._consumeLastLookaheadStr();
...
_[DOCTYPE_PUBLIC_IDENTIFIER_DOUBLE_QUOTED_STATE] = function (ch) {
    if (ch === '"')
        ...
        this._err(err.UNEXPECTED_NULL_CHARACTER);
        this.currentToken.publicIdentifier += NULL_REPLACEMENT;
    }
        ...
        this.currentToken.publicIdentifier += ch;
};
...
_[DOCTYPE_SYSTEM_IDENTIFIER_DOUBLE_QUOTED_STATE] = function (ch) {
    if (ch === '"')
        ...
        this._err(err.UNEXPECTED_NULL_CHARACTER);
        this.currentToken.systemIdentifier += NULL_REPLACEMENT;
    }
        ...
        this.currentToken.systemIdentifier += ch;
};

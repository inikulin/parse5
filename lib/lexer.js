...
Lexer.prototype._shelveNewStartTagToken = function (firstTagNameCh) {
    this.shelvedToken = {
...
Lexer.prototype._shelveEndTagToken = function (firstTagNameCh) {
    this.shelvedToken = {
...
_[TAG_OPEN_STATE] = function (ch) {
    if (ch === '!')
        ...
        this._shelveNewStartTagToken(asciiToLower(ch));
        this.state = TAG_NAME_STATE;
    } else if (ch >= 'a' && ch <= 'z') {
        this._shelveNewStartTagToken(ch);
        this.state = TAG_NAME_STATE;
    } else if (ch === '?') {
        this._err(err.BOGUS_COMMENT);
        this.state = BOGUS_COMMENT_STATE;
    } else {
        this._err(err.BOGUS_TAG_DECLARATION);
        this._emitCharacterToken('<');
        this._reconsume();
        this.state = DATA_STATE;
    }
...
_[END_TAG_OPEN_STATE] = function (ch) {
    if (ch >= 'A' && ch <= 'Z') {
        this._shelveEndTagToken(asciiToLower(ch));
        this.state = TAG_NAME_STATE;
    } else if (ch >= 'a' && ch <= 'z') {
        this._shelveEndTagToken(ch);
        this.state = TAG_NAME_STATE;
    } else if (ch === '>') {
        this._err(err.MISSING_END_TAG_NAME);
        this.state = DATA_STATE;
    } else if (ch === EOF) {
        this._err(err.EOF_IN_END_TAG_NAME);
        this._emitCharacterToken('<');
        this._emitCharacterToken('/');
        this._reconsume();
        this.state = DATA_STATE;
